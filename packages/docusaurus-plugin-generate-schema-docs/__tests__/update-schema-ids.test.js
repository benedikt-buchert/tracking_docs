...
describe('updateSchemaIds', () => {
    ...
    it('should update the $id of the schemas in the versioned directories', () => {
        ...
        const schemaPath = path.join(tempSiteDir, 'static/schemas', '1.1.1', 'add-to-cart-event.json');
        const schema = JSON.parse(fs.readFileSync(schemaPath, 'utf8'));

        expect(schema.$id).toBe('https://tracking-docs-demo.buchert.digital/schemas/1.1.1/add-to-cart-event.json');
    });
});
