...
jest.mock('../../components/ExampleDataLayer', () => {
  const MockExampleDataLayer = () => <div>Mocked ExampleDataLayer</div>;
  MockExampleDataLayer.displayName = 'MockExampleDataLayer';
  return MockExampleDataLayer;
});
jest.mock('../../components/PropertiesTable', () => {
  const MockPropertiesTable = () => <div>Mocked PropertiesTable</div>;
  MockPropertiesTable.displayName = 'MockPropertiesTable';
  return MockPropertiesTable;
});
...
describe('SchemaViewer', () => {
  ...
  it('renders the schema viewer with "DataLayer Examples" for a schema with choices', () => {
    const schema = {
      oneOf: [{ type: 'object', properties: { event: { type: 'string' } } }],
    };
