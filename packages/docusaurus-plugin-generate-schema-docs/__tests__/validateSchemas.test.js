...
describe('validateSchemas', () => {
  ...
  it('should return true when all schemas are valid', async () => {
    ...
    expect(consoleLogSpy).toHaveBeenCalledWith(
      '✅ Schema valid-schema.json produced a valid example.',
    );
  });
  ...
  it('should return false if an example fails validation', async () => {
    ...
    expect(consoleErrorSpy).toHaveBeenCalledWith(
      '❌ Schema invalid-example-schema.json example data failed validation:',
    );
    expect(consoleErrorSpy).toHaveBeenCalledWith(
  ...
  it('should fail validation for missing required property', async () => {
    ...
    expect(result).toBe(false);
    expect(consoleErrorSpy).toHaveBeenCalledWith(
      '❌ Schema no-valid-example-schema.json example data failed validation:',
    );
    expect(consoleErrorSpy).toHaveBeenCalledWith(
      expect.arrayContaining([
        expect.objectContaining({
  ...
  it('should handle schemas with $refs correctly', async () => {
    ...
    expect(result).toBe(true);
    expect(consoleLogSpy).toHaveBeenCalledWith(
      '✅ Schema main-schema-with-ref.json produced a valid example.',
    );
  });
