...
const OneOfAnyOfRow = ({ propertyKey, prop, isReq }) => {
  const choices =
    prop.oneOf ||
    prop.anyOf ||
    (prop.items && (prop.items.oneOf || prop.items.anyOf));
  ...
  return (
    <tr className={clsx(isReq && 'required-row')}>
      ...
      <td>
        <code>{getPropertyType(uniqueTypes)}</code>
      </td>
...
const DefaultRow = ({ propertyKey, prop, isReq, getConstraints }) => {
  ...
  const childProperties =
    prop.properties || (prop.items && prop.items.properties);
  const hasChildren =
  ...
  return (
    <React.Fragment>
      <tr className={clsx(isReq && 'required-row')}>
        ...
        <td rowSpan={numRows}>
          <code>{getPropertyType(prop.type)}</code>
        </td>
        <td>
          {firstConstraint && (
            <code
        ...
        <td rowSpan={numRows}>{formatExamples(prop.examples)}</td>
        <td rowSpan={numRows}>{prop.description || ''}</td>
      </tr>

      {remainingConstraints.map((constraint, index) => (
        <tr
          className={clsx(isReq && 'required-row')}
          key={`${propertyKey}-constraint-${index}`}
        >
          <td>
            <code
              className={clsx(
...
const PropertyRow = ({ propertyKey, prop, requiredList, getConstraints }) => {
  ...
  if (hasOneOf || hasAnyOf) {
    return (
      <OneOfAnyOfRow propertyKey={propertyKey} prop={prop} isReq={isReq} />
    );
  }
